  ...
  (deftest threeten-extra-test
    ...
    (testing "ordering"
      (let [interval-1-2 (j/interval 1 2)
        ...
        (is-antisymmetric (j/<= interval-3-4 interval-3-4))
        (is (not (j/>= interval-1-2 interval-3-4)))
        (is (not (j/>= interval-3-4 instant-3)))
        (is (not (j/>= instant-3 interval-3-4)))
        (is-antisymmetric (j/>= interval-3-4 instant-1))
        (is-antisymmetric ((complement j/>=) interval-1-2 instant-3))

        (is-antisymmetric (j/<= interval-1-2 interval-1-2))
        (is-antisymmetric (j/<= interval-1-2 interval-3-4))
        (is (not (j/<= interval-1-2 instant-1)))
        (is (not (j/<= instant-1 interval-1-2)))
        (is (not (j/<= instant-1 interval-1-2 instant-2)))
        (is (not (j/<= instant-1 interval-1-2 instant-2 interval-1-2 instant-1)))
        (is (not (j/<= instant-1 interval-1-2 interval-1-2 instant-1)))
        (is-antisymmetric (j/<= interval-1-2 instant-3))
        (is-antisymmetric ((complement j/<=) interval-3-4 instant-1)))
      (testing "transitivity"
        (let [interval-1-2 (j/interval 1 2)
              ...
              instant-2 (j/instant 2)
              instant-3 (j/instant 3)]
          (is (not (j/<= instant-1 instant-2 instant-1)))
          (is (not (j/<= instant-1 interval-1-3)))
          (is (not (j/<= instant-1 interval-1-3)))
          (is (not (j/<= instant-1 interval-1-3 instant-2 interval-1-3 instant-1)))
          (is (not (j/< instant-1 interval-1-3 instant-2 interval-1-3 instant-1)))
          (is (not (j/<= instant-2 interval-1-3 instant-1)))
          (is (not (j/>= instant-1 interval-1-3 instant-2)))
          (is (not (j/<= instant-1 interval-1-3 instant-2)))
          (is (not (j/<= instant-1 interval-1-3)))
          ;; instant-1 and interval-1-3 are unrelated, which does not work with
          ;; implementations of <= based on negation
          (is (not (j/after? instant-1 interval-1-3)))
          (is (not (j/after? interval-1-3 instant-1)))
          (is (not (j/before? instant-1 interval-1-3)))
