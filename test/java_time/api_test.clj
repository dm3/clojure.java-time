(ns java-time.api-test
  "Shares tests with java-time-test. Don't
  add requires or imports to the ns form---only
  set the meaning of namespace alias j."
...
(deftest ordering-test
  ...
  (testing "times"
    (let [ldt (j/local-date-time clock)
      ...
      (is (j/after? ldt+10 ldt+5 ldt))
      (is (not (j/after? ldt ldt+5 ldt+10)))
      (is (j/before? ldt ldt+5 ldt+10))
      (is (not (j/before? ldt+10 ldt+5 ldt)))
      (is (j/not-after? ldt ldt+5 ldt+10))
      (is (j/not-before? ldt+10 ldt+5 ldt))
      (is (not (j/not-after? ldt ldt+10 ldt+5)))
      (is (not (j/not-before? ldt+10 ldt ldt+5))))

    (let [ld (j/local-date clock)
          ld+5 (j/plus ld (j/days 5))
          ld+10 (j/plus ld (j/days 10))]
      (is (j/after? ld))
      (is (j/before? ld))
      (is (j/not-after? ld))
      (is (j/not-before? ld))
      ...
      (is (j/after? ld+10 ld+5 ld))
      (is (not (j/after? ld ld+5 ld+10)))
      (is (j/before? ld ld+5 ld+10))
      (is (not (j/before? ld+10 ld+5 ld)))
      (is (j/not-after? ld ld+5 ld+10))
      (is (j/not-before? ld+10 ld+5 ld))
      (is (not (j/not-after? ld+10 ld ld+5)))
      (is (not (j/not-before? ld ld+10 ld+5))))

    (let [lt (j/local-time clock)
          lt+5 (j/plus lt (j/minutes 5))
          lt+10 (j/plus lt (j/minutes 10))]
      (is (j/after? lt))
      (is (j/before? lt))
      (is (j/not-after? lt))
      (is (j/not-before? lt))
      ...
      (is (j/after? lt+10 lt+5 lt))
      (is (j/before? lt lt+5 lt+10))
      (is (j/not-after? lt lt+5 lt+10))
      (is (j/not-before? lt+10 lt+5 lt))
      (is (not (j/not-after? lt+10 lt lt+5)))
      (is (not (j/not-before? lt lt+10 lt+5))))

    (let [zdt (j/zoned-date-time clock)
          zdt+5 (j/plus zdt (j/minutes 5))
          zdt+10 (j/plus zdt (j/minutes 10))]
      (is (j/after? zdt))
      (is (j/before? zdt))
      (is (j/not-after? zdt))
      (is (j/not-before? zdt))
      ...
      (is (j/after? zdt+10 zdt+5 zdt))
      (is (j/before? zdt zdt+5 zdt+10))
      (is (j/not-after? zdt zdt+5 zdt+10))
      (is (j/not-before? zdt+10 zdt+5 zdt))
      (is (not (j/not-after? zdt+10 zdt zdt+5)))
      (is (not (j/not-before? zdt zdt+10 zdt+5))))

    (let [odt (j/offset-date-time clock)
          odt+5 (j/plus odt (j/minutes 5))
          odt+10 (j/plus odt (j/minutes 10))]
      (is (j/after? odt))
      (is (j/before? odt))
      (is (j/not-after? odt))
      (is (j/not-before? odt))
      ...
      (is (j/after? odt+10 odt+5 odt))
      (is (j/before? odt odt+5 odt+10))
      (is (j/not-after? odt odt+5 odt+10))
      (is (j/not-before? odt+10 odt+5 odt))
      (is (not (j/not-after? odt+10 odt odt+5)))
      (is (not (j/not-before? odt odt+10 odt+5))))

    (let [ot (j/offset-time clock)
          ot+5 (j/plus ot (j/minutes 5))
          ot+10 (j/plus ot (j/minutes 10))]
      (is (j/after? ot))
      (is (j/before? ot))
      (is (j/not-after? ot))
      (is (j/not-before? ot))
      ...
      (is (j/after? ot+10 ot+5 ot))
      (is (j/before? ot ot+5 ot+10))
      (is (j/not-after? ot ot+5 ot+10))
      (is (j/not-before? ot+10 ot+5 ot))
      (is (not (j/not-after? ot+10 ot ot+5)))
      (is (not (j/not-before? ot ot+10 ot+5))))

    (let [i (j/instant clock)
          i+5 (j/plus i (j/minutes 5))
          i+10 (j/plus i (j/minutes 10))]
      (is (j/after? i))
      (is (j/before? i))
      (is (j/not-after? i))
      (is (j/not-before? i))
      ...
      (is (j/after? i+10 i+5 i))
      (is (j/before? i i+5 i+10))
      (is (j/not-after? i i+5 i+10))
      (is (j/not-before? i+10 i+5 i))
      (is (not (j/not-after? i+10 i i+5)))
      (is (not (j/not-before? i i+10 i+5)))))
  ...
  (testing "clocks"
    (let [fc (j/fixed-clock 0)
      ...
      (is (j/after? fc+2000 fc+1000 fc))
      (is (j/before? fc fc+1000 fc+2000))
      (is (j/not-after? fc fc+1000 fc+2000))
      (is (j/not-before? fc+2000 fc+1000 fc))
      (is (not (j/not-after? fc+2000 fc fc+1000)))
      (is (not (j/not-before? fc fc+2000 fc+1000)))))
  ...
  (testing "fields"
    (let [thursday (j/day-of-week :thursday)
      ...
      (is (j/after? thursday))
      (is (j/before? thursday))
      (is (j/not-after? thursday))
      (is (j/not-before? thursday))
      ...
      (is (j/after? sunday saturday thursday))
      (is (j/before? thursday saturday sunday))
      (is (j/not-after? thursday saturday sunday))
      (is (j/not-before? sunday saturday thursday))
      (is (not (j/not-after? sunday thursday saturday)))
      (is (not (j/not-before? thursday sunday saturday))))
    ...
    (let [january (j/month :january)
      ...
      (is (j/after? january))
      (is (j/before? january))
      (is (j/not-after? january))
      (is (j/not-before? january))
      ...
      (is (j/after? march february january))
      (is (j/before? january february march))
      (is (j/not-after? january february march))
      (is (j/not-before? march february january))
      (is (not (j/not-after? march january february)))
      (is (not (j/not-before? january march february))))

    (let [year-2009 (j/year 2009)
          year-2010 (j/year 2010)
          year-2011 (j/year 2011)]
      (is (j/after? year-2009))
      (is (j/before? year-2009))
      (is (j/not-after? year-2009))
      (is (j/not-before? year-2009))
      ...
      (is (j/after? year-2011 year-2010 year-2009))
      (is (j/before? year-2009 year-2010 year-2011))
      (is (j/not-after? year-2009 year-2010 year-2011))
      (is (j/not-before? year-2011 year-2010 year-2009))
      (is (not (j/not-after? year-2011 year-2009 year-2010)))
      (is (not (j/not-before? year-2009 year-2011 year-2010))))

    (let [jan-1 (j/month-day 1 1)
          apr-1 (j/month-day 4 1)
          aug-1 (j/month-day 8 1)]
      (is (j/after? jan-1))
      (is (j/before? jan-1))
      (is (j/not-after? jan-1))
      (is (j/not-before? jan-1))
      ...
      (is (j/after? aug-1 apr-1 jan-1))
      (is (j/before? jan-1 apr-1 aug-1))
      (is (j/not-after? jan-1 apr-1 aug-1))
      (is (j/not-before? aug-1 apr-1 jan-1))
      (is (not (j/not-after? aug-1 jan-1 apr-1)))
      (is (not (j/not-before? jan-1 aug-1 apr-1))))))
    ...
    (testing "ordering"
      ...
      (testing "instant<interval"
        ...
        (is (not (j/before? (j/instant 1999) (j/interval (j/instant 1000) (j/instant 2000))))
            "before end")
        (is (not (j/before? (j/instant 2000) (j/interval (j/instant 1000) (j/instant 2000))))
            "at end")
        (is (not (j/before? (j/instant 3000) (j/interval (j/instant 1000) (j/instant 2000))))
            "after end"))
      (testing "instant>interval"
        (is (not (j/after? (j/instant 100) (j/interval (j/instant 1000) (j/instant 2000))))
            "before start")
        (is (not (j/after? (j/instant 1000) (j/interval (j/instant 1000) (j/instant 2000))))
            "at start")
        (is (not (j/after? (j/instant 1500) (j/interval (j/instant 1000) (j/instant 2000))))
            "middle")
...
(deftest formatter-test
  (testing "case-insensitive formatter"
    (let [fmt (j/formatter "hh:mma" {:case :insensitive})]
      (is (= (j/local-time 0 34 0 0)
             (j/local-time fmt "00:34am")
             (j/local-time fmt "00:34AM")))))

  (testing "case-sensitive formatter"
    (let [fmt (j/formatter "hh:mma" {:case :sensitive})]
      (is (= (j/local-time 0 34 0 0)
